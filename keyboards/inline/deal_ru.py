...
go_basked = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(text='Перейти в корзину', callback_data='gobaskedcalru')
        ]
    ]
)
...
def product_items(count=1, product_id=1):
    keyb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text='➖5️⃣', callback_data=f'productitemsru_-5_{product_id}'),
                InlineKeyboardButton(text='➖1️⃣', callback_data=f'productitemsru_-1_{product_id}'),
                InlineKeyboardButton(text=str(count), callback_data=f'productitemsru_self_{product_id}'),
                InlineKeyboardButton(text='➕1️⃣', callback_data=f'productitemsru_1_{product_id}'),
                InlineKeyboardButton(text='➕5️⃣', callback_data=f'productitemsru_5_{product_id}'),
            ],
            [
                InlineKeyboardButton(text='💾Сохранить', callback_data=f'productitemsru_save_{product_id}')
            ],
            [
                InlineKeyboardButton(text='⬅Назад', callback_data=f'productitemsru_back_{product_id}')
            ],
            [
                InlineKeyboardButton(text='⬅Вернуться в главное меню', callback_data='headbackru')
            ]
        ]
    )
    return keyb
...
def product_keyboard(obj):
    keyb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text=i.name, callback_data=f'productfirstru_{i.id}')
            ] for i in obj
        ]
    )
    keyb.inline_keyboard.append([InlineKeyboardButton(text='⬅Вернуться в главное меню', callback_data='headbackru')])
    return keyb
