...
go_basked = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(text='Savatga o\'tish', callback_data='gobaskedcall')
        ]
    ]
)


def product_items(count=1, product_id=1):
    keyb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text='➖5️⃣', callback_data=f'productitems_-5_{product_id}'),
                InlineKeyboardButton(text='➖1️⃣', callback_data=f'productitems_-1_{product_id}'),
                InlineKeyboardButton(text=str(count), callback_data=f'productitems_self_{product_id}'),
                InlineKeyboardButton(text='➕1️⃣', callback_data=f'productitems_1_{product_id}'),
                InlineKeyboardButton(text='➕5️⃣', callback_data=f'productitems_5_{product_id}'),
            ],
            [
                InlineKeyboardButton(text='💾Saqlash', callback_data=f'productitems_save_{product_id}')
            ],
            [
                InlineKeyboardButton(text='⬅Orqaga', callback_data=f'productitems_back_{product_id}')
            ],
            [
                InlineKeyboardButton(text='⬅Bosh menyuga qaytish', callback_data='headback')
            ]
        ]
    )
    return keyb
...
def product_keyboard(obj):
    keyb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text=i.name, callback_data=f'productfirst_{i.id}')
            ] for i in obj
        ]
    )
    keyb.inline_keyboard.append([InlineKeyboardButton(text='⬅Bosh menyuga qaytish', callback_data='headback')])
    return keyb
...
add_clinic_keyb = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(text='Klinikani yoki Vet Aptekani yoki Laboratoriyani qo`shish',
                                 callback_data='addclinic')
        ]
    ]
)
