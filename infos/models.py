...
class UserStatusManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(status_registration=True)
...
class Basked(models.Model):
    user = models.ForeignKey(Users, on_delete=models.CASCADE)

    objects = models.Manager()


class BaskedItem(models.Model):
    basked = models.ForeignKey(Basked, on_delete=models.CASCADE, null=True,related_name='basked_set')
    ...
    def save(
        ...
        if self.product:
            self.total = self.product.price * self.count_product
            super(BaskedItem, self).save()


class OrderSendUser(models.Model):
